...
#define SX126X_SLEEP_START_COLD                                                                    \
    0b00000000 //  2     2     sleep mode: cold start, configuration is lost (default)
...
#define SX126X_GFSK_PREAMBLE_DETECT_OFF                                                            \
    0x00 //  7     0     GFSK minimum preamble length before reception starts: detector disabled
...
#define SX126X_GFSK_PACKET_FIXED                                                                   \
    0x00 //  7     0     GFSK packet type: fixed (payload length known in advance to both sides)
...
#define SX126X_STATUS_DATA_AVAILABLE                                                               \
    0b00000100 //  3     1     command status: packet received and data can be retrieved
...
void SPItransfer(uint8_t cmd, bool write, uint8_t* dataOut, uint8_t* dataIn, uint8_t numBytes,
                 bool waitForBusy);
void LoRaError(int error);

#endif // __API_LORA_H__